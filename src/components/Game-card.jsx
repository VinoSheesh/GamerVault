...
export default function GameCard({ game, hoveredCardId, onHover, onLikeToggle, likedGames, onAddToCart, onGameClick }) {
  ...
  const handleCartClick = (e) => {
    ...
    button.classList.add("cart-button-clicked")
    ...
    setTimeout(() => {
      button.classList.remove("cart-button-clicked")
    }, 300)
  }
  ...
  return (
    ...
    >
      <div className="relative w-full pb-[125%]">
        ...
        />
        {isNewGame(game._id) && (
          <div className="absolute top-3 left-3 bg-white text-black text-xs font-bold px-2 py-1 rounded">New</div>
        )}

        {/* Cart Button */}
        <button
          className={`absolute bottom-3 right-3 p-2 rounded-full bg-blue-600 hover:bg-blue-700 transition-all duration-200 ${
            hoveredCardId === game._id ? "opacity-100 scale-100" : "opacity-0 scale-95"
          }`}
        ...
        <button
          className={`absolute top-3 right-3 p-2 rounded-full bg-gray-900 bg-opacity-70 transition-opacity duration-200 ${
            hoveredCardId === game._id || likedGames.includes(game._id) ? "opacity-100" : "opacity-0"
          }`}
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
        ...
        >
          <Heart size={20} fill={likedGames.includes(game._id) ? "white" : "none"} color="white" />
        </button>
      </div>
      ...
      <div className="p-3">
        ...
        {/* Rating stars */}
        <div className="mb-2">{renderRatingStars(game.rating)}</div>
        ...
        <div className="flex items-center flex-wrap mt-2">
          {game.discount > 0 ? (
            <>
              ...
              <span className="text-gray-400 line-through text-xs mr-1">${game.price.toFixed(2)}</span>
